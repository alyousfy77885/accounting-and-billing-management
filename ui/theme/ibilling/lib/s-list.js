$(document).ready(function () {
    ...
    sysrender.on('click', '.cdelete', function(e){
        ...
        bootbox.confirm(lan_msg, function(result) {
            if(result){
                var _url = $("#_url").val();
                window.location.href = _url + "delete/ss/" + id + '/';
            }
        });
    });
    ...
    sysrender.on('click', '.cedit', function(e){
       ...
       // var id = $(this).closest('tr').attr('id');
        ...
        var _url = $("#_url").val();
        setTimeout(function(){
            $modal.load(_url + 'stock/edit-c/' + id, '', function(){
                $modal.modal();
            });
        }, 1000);
    });
    ...
    $modal.on('click', '#update', function(){
        ...
        setTimeout(function(){
            ...
            var _url = $("#_url").val();
            $.post(_url + 'stock/edit-f/', $('#edit_for').serialize(), function(data){
    ...
    $modal.on('click', '#update-c', function(){
        $modal.modal('loading');
        setTimeout(function(){


            var _url = $("#_url").val();
            $.post(_url + 'stock/edit-c/', $('#edit_for').serialize(), function(data){
