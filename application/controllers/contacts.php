    ...
    case 'activity':
        ...
        if ($d) {
            $ac = ORM::for_table('sys_activity')
                ...
                ->order_by_desc('id')
                ->find_many();
            $ui->assign('ac', $ac);$ui->assign('type', 'cust');
    ...
    case 'transactions':
        ...
        if ($d) {
            $tr = ORM::for_table('sys_transactions')
                ...
                ->order_by_desc('id')
                ->find_many();
            $ui->assign('tr', $tr);$ui->assign('type', 'cust');
...
$ui->assign('type', 'cust');
 ...
 ' . $extra_jq
    ...
    case 'add-post':
     ...
     $balance=566;
     $d1 = ORM::for_table('sys_transactions')->create();
...
*/
                ...
                $d1->aid = 0;
                $d1->updated_at = date('Y-m-d H:i:s');

                $d1->save();

 $d2 = ORM::for_table('sys_accounts')->create();
          $d2->account =$account;
         $d2->description = $address;
            $d2->balance = $balance;
            // From Version 4

            $d2->bank_name = '';
            $d2->account_number =_post('account_number');
            $d2->currency = '';
            $d2->branch = '';
            $d2->address = '';
        $d2->contact_person =$email;
            $d2->contact_phone = $phone;
            $d->website = '';
            $d2->ib_url ='gabbb';
            $d2->created =  date('Y-m-d H:i:s');
            //date('Y-m-d H:i:s');
            $d2->notes = '';
            $d2->sorder = 1;
            $d2->e = '';
