...
switch ($action) {
 ...
 '
    ...
    case 'add-post':
        ...
        $msg = '';
        if ($cid == '') {
            $msg .= $_L['select_a_contact'] . ' yhh<br> ';
        }
        ...
        if (isset($_POST['amount'])) {
            ...
            $msg .= $_L['at_least_one_item_required'] . '44 <br> ';
        }
...
$cod=$code[$i];
  ...
  $e = ORM::for_table('stock')
                ->where('product', $item)
                ->find_one();

$msg="habsv";
     if($e->count>$sqty){
         //$e->count=$e->count-$sqty;
                ...
                $d->save();
                $i++;
  $ee = ORM::for_table('stock')
                ->where('product', $item)
                ->find_one();
     if($ee){
         $ee->count=$ee->count-$sqty;
         $ee->save();
...
$(".cdelete").click(function (e) {
 ...
 '
  ...
  $dr = ORM::for_table('sys_invoiceitems')
...
foreach($dr as $dd){
  $k= ORM::for_table('stock')->where('pid',$dd['itemcode'])->find_one();
               
                
     if($k){
         $k->count=$k->count+$dd['qty'];
         $k->save();
